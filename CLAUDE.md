...
> 实现状态：以上是 V1.7 目标规则；现有代码仍是 V1.5/V1.6 行为（15分钟预占、
> 6位码、按件成团等），尚未按 V1.7 改造。ACTIONS.md 仍是 V1.6 契约、待重修。
> 不得因为文档已更新就声称代码已经完成。
...
- 完成 PRD V1.7 代码改造：3分钟预占、5人成团(按人)、22:00锁定、12:00确认+
  成团后手动取消、两段制退款、手机尾号核销+尾号重复预警、交付拍照、双交付模式、
  发布只选SKU+库存+勾站、10点软截止、休息态+催开团计数、权限多站
- 同步重修 `cloudfunctions/api/ACTIONS.md` 到 V1.7 契约（现仍为 V1.6）
- 上线前：删除 adminBatch/product 页的 demo 兜底数据；申请2个订阅模板并填 config
