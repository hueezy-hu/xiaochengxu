...
test('checkout creates one multi-SKU reservation and payment owns the countdown', () => {
  ...
  assert.match(checkout, /amountText:\s*app\.money\(amount\)/)
...
test('station sharing lands on the V1.7 group page without locking a SKU', () => {
  const group = read('miniprogram/pages/groupPage/groupPage.js')
  const success = read('miniprogram/pages/paySuccess/paySuccess.js')
  const detail = read('miniprogram/pages/orderDetail/orderDetail.js')
  assert.match(group, /onShareAppMessage/)
  assert.match(group, /onShareTimeline/)
  assert.match(group, /pages\/groupPage\/groupPage\?batchStationId=/)
  assert.match(success, /pages\/groupPage\/groupPage\?batchStationId=/)
  assert.match(detail, /pages\/groupPage\/groupPage\?batchStationId=/)
